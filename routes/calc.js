...
router.post('/:id', ensureAuth, async (req, res) => {
    try {
        ...
        let user = await Team.findById(req.body.person).lean()
        ...
        if (user.worldSolved == question.worldNumber) {
            if (user.questionSolved+1 == question.questionNo) {
                ...
                if(!(question.worldNumber == 5 && question.questionNo == 9)){
                ...
                if(question.worldNumber == 5 && question.questionNo == 9)
                {
                    await Team.update({_id: req.body.person}, {$inc:{q9: 1}})
                    if(user.q9+1 >= 5)
                    flagg = 0
                }
                if (flagg>=0) {
                    ...
                    userr = await Team.findOneAndUpdate({ _id: req.body.person },  { 
                        score: sco,
                        questionSolved: qSol,
                        worldSolved: wSol,
                        worldNumber: wNo,
